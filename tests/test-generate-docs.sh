...
for TYPE in $(ls -d docs/*); do
  ...
  if [ -e $REAL_DEST/current ] && [ -e $TEST_DEST/current ]; then
    diff -r $REAL_DEST/current $TEST_DEST/current --exclude="*.tsv" --exclude="*.xlsx" --exclude="doi-object.zip" \
  ...
  if [ -e $REAL_DEST/deprecated ] && [ -e $TEST_DEST/deprecated ]; then
    diff -r $REAL_DEST/deprecated $TEST_DEST/deprecated --exclude="*.tsv" --exclude="*.xlsx" --exclude="doi-object.zip" \
      || die "Update needed: $REAL_CMD
