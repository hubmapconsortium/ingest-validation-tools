...
This is the most recent metadata specification that needs to be followed for the submission of new data. See the [harmonized specification](https://docs.hubmapconsortium.org/assays/metadata/visium-hd.html) for a view of the metadata across this and any previous versions.
...
| <code>lab_processed\/spaceranger\/feature_slice\.h5$</code> | ✓ | HDF5 file containing Visium HD gene expression data, structured for easy and efficient retrieval of 2 µm resolution "image" slices for single or multiple genes. The data can be binned to arbitrary scales or plotted against the microscope tissue image. |
| <code>lab_processed\/spaceranger\/barcode_mappings\.parquet$</code> |  | Contains spatial mapping information that links sequencing barcodes to physical locations and segmented cells. |
...
| <code>lab_processed\/spaceranger\/segmented_outputs\/.*</code> | ✓ | Segmented output from the 10X Genomics SpaceRanger pipeline. |
| <code>lab_processed\/spaceranger\/segmented_outputs\/cell_segmentations\.geojson$</code> |  | Contains a spatial collection of polygons that define the exact physical boundaries of individual cells in a tissue sample. |
| <code>lab_processed\/spaceranger\/segmented_outputs\/nucleus_segmentations\.geojson$</code> |  | Contains spatial mapping output containing polygon coordinates that outline the exact boundaries of individual cell nuclei detected within a tissue sample. |
| <code>lab_processed\/spaceranger\/segmented_outputs\/filtered_feature_bc_matrix\.h5$</code> |  | HDF5 file that stores the quantified, tissue-associated gene (or feature) expression data for a spatial transcriptomics experiment. It acts as the primary input for downstream spatial analysis in tools like Seurat or spatialGE. |
| <code>lab_processed\/spaceranger\/segmented_outputs\/spatial\/.*</code> | ✓ | Segmented spatial output from the 10X Genomics SpaceRanger pipeline. |
| <code>lab_processed\/spaceranger\/segmented_outputs\/spatial\/scalefactors_json\.json$</code> |  | Contains the scaling and resizing parameters needed to map transcriptomic data onto tissue microscopy images. It provides the exact multipliers and spot dimensions required to align spot barcodes with image pixels. |
